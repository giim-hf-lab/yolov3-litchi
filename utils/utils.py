...
def scale_coords_re(img1_shape, coords, img0_shape, ratio_pad=None):
    # Rescale coords (xyxy) from img1_shape to img0_shape
...
#**********************Revision***************#
def ap_per_class_re(tp, conf, pred_cls, target_cls, eps=1e-16):
    """ Compute the average precision, given the recall and precision curves.
    Source: https://github.com/rafaelpadilla/Object-Detection-Metrics.
    # Arguments
    ...
    for ci, c in enumerate(unique_classes):
        ...
        # print('*********pred_cls=',pred_cls)
        i = pred_cls == c
        n_gt = (target_cls == c).sum()  # number of labels
        ...
        if n_p == 0 or n_gt == 0:
            ...
            for j in range(tp.shape[1]):
                ap[ci, j], mpre, mrec = compute_ap_re(recall[:, j], precision[:, j])
                if j == 0:
    ...
    # print('*****p_mean=', p.mean())
    p, r, f1 = p[:, i], r[:, i], f1[:, i]
    return p, r, f1, ap, unique_classes.astype('int32')

def compute_ap_re(recall, precision):
    """ Compute the average precision, given the recall and precision curves
    # Arguments
        recall:    The recall curve (list)
        precision: The precision curve (list)
    ...
    # Append sentinel values to beginning and end
    # print("****reccall_shape=",recall)
    # print("****precision_shape=",precision)
...
def non_max_suppression_re(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
                        labels=(), max_det=300):
